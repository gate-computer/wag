...
func (MacroAssembler) Enter(p *gen.Prog) {
	...
	if p.Text.Addr != nonabi.TextAddrRetpoline {
		panic("amd64: hardcoded retpoline address needs to be adjusted")
	}
	...
	if p.Text.Addr != nonabi.TextAddrRetpolineSetup {
		panic("amd64: hardcoded retpoline setup address needs to be adjusted")
	}
	...
	case wa.Int:
		...
		case storage.Reg:
			if source := x.Reg(); source != target {
				...
				if target != RegResult {
					panic("amd64: register moved to itself")
				}
			}
